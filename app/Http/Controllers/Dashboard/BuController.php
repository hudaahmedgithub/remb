...
{
	$bus=$bu->where('statu_id',1)->orderBy('id','desc')->paginate(5);
	return view('rent',compact('bus'));
}
	
	public function own(Bu $bu)
{
	$bus=$bu->where('bu_status',1)->where('statu_id',2)->orderBy('id','desc')->paginate(5);
  ...
  public function update(Request $request,Bu $bu)
	...
	$request->validate([
	   ...
	   if ($request->image) {
		...
		$bu->save();
		return redirect()->route('dashboard.bu.index');
    }
    ...
    {
      Bu::find($id)->delete();
		return redirect('/dashboard/bu')->withFlashMessage('تم الحذف بنجاح');
    }
